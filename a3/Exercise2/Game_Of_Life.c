...
int main (int argc, char * argv[]) {
	...
	if((!strcmp(argv[1], "-h")) && argc == 2) {
			printf("Run the program by typing ./main [Array dimension] [Time steps] [Amount of threads]\n");
			printf("To compile the program, type \"make\" (in this directory, where you are standing)\n");
			exit(0);
	}
	...
	for (t = 0 ; t < T ; t++) {
		...
		#pragma omp parallel num_threads (threads) private(i,j,nbrs)
		{
			#pragma omp for
			for (i = 1 ; i < N-1 ; i++) {
				for (j = 1 ; j < N-1 ; j++) {
